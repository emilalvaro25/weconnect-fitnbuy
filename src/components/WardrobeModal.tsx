...
interface WardrobePanelProps {
  ...
  onAccessorySelect: (accessoryFile: File, accessoryInfo: WardrobeItem) => void;
...
const WardrobePanel: React.FC<WardrobePanelProps> = ({ onGarmentSelect, onAccessorySelect, activeGarmentIds, isLoading, wardrobe, accessories }) => {
    const [error, setError] = useState<string | null>(null);

    const handleItemClick = async (item: WardrobeItem, onSelect: (file: File, info: WardrobeItem) => void) => {
        ...
        <div>
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                return (
                    <button
                    key={item.id}
                    onClick={() => handleItemClick(item, onGarmentSelect)}
        ...
        <div>
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                return (
                    <button
                    key={item.id}
                    onClick={() => handleItemClick(item, onAccessorySelect)}
                    disabled={isLoading || isActive}
                    className="relative aspect-square border rounded-lg overflow-hidden transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 group disabled:opacity-60 disabled:cursor-not-allowed"
                    aria-label={`Select ${item.name}`}
                    >
            ...
            {accessories.length === 0 && (
                <p className="text-center text-sm text-gray-500 mt-4">Accessories will appear here.</p>
            )}
        </div>
